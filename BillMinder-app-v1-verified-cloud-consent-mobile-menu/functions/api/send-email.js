...
export async function onRequestPost({ request, env }) {
  ...
  const preferences = await getReminderPreferences(env, user.id, authToken);
  if (!preferences.emailReminders) {
    return jsonResponse({ error: "Email reminders are turned off for this account." }, 403);
  }
...
async function getReminderPreferences(env, userId, authToken) {
  const query = `/rest/v1/user_preferences?user_id=eq.${encodeURIComponent(userId)}&select=email_reminders_enabled&limit=1`;
  const response = await supabaseFetch(env, query, {
    headers: { Authorization: `Bearer ${authToken}` }
  });

  if (!response.ok) return { emailReminders: false };
  const rows = await response.json().catch(() => []);
  return { emailReminders: Boolean(rows[0]?.email_reminders_enabled) };
}
