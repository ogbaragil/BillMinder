...
function setupSettings() {
  ...
  els.reminderLeadSelect.addEventListener("change", () => {
    ...
    saveSettings();
    syncRemotePreferences({ silent: true });
  });
...
async function authenticate(mode, source) {
  ...
  try {
    ...
    updateAuthStatus();
    updateAuthGate();
    await restoreRemotePreferences({ silent: true });
  } catch (error) {
...
async function syncRemotePreferences({ silent = false } = {}) {
  if (!hasSyncConnection()) return;

  try {
    await upsertRemotePreferences();
    if (!silent) updateSyncStatus("Reminder preferences synced.");
  } catch (error) {
    if (!silent) updateSyncStatus(error.message || "Preference sync failed.");
  }
}

async function restoreRemotePreferences({ silent = false } = {}) {
  if (!hasSyncConnection()) return;

  try {
    const preferences = await fetchRemotePreferences();
    if (preferences) {
      state.settings.emailReminders = Boolean(preferences.emailReminders);
      const restoredLeadDays = Number(preferences.reminderLeadDays ?? state.settings.reminderLeadDays);
      state.settings.reminderLeadDays = [0, 1, 3, 7].includes(restoredLeadDays) ? restoredLeadDays : state.settings.reminderLeadDays;
      saveSettings();
      renderReminderPreferences();
      if (!silent) updateSyncStatus("Reminder preferences restored.");
    }
  } catch (error) {
    if (!silent) updateSyncStatus(error.message || "Preference restore failed.");
  }
}

async function upsertRemotePreferences() {
  const response = await cloudflareSyncRequest("/api/preferences", {
    method: "POST",
    body: JSON.stringify({
      preferences: {
        emailReminders: Boolean(state.settings.emailReminders),
        reminderLeadDays: Number(state.settings.reminderLeadDays) || 3
      }
    })
  });
